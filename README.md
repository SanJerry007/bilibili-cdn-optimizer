...
它把真实的脚本文件加载进一个桩浏览器（假的 fetch / XHR / localStorage / 时钟），断言的是行为不是结构。49 条断言里，34 条在 2.1 上是红的、3 条在 3.1 上是红的，所以这个闸门不会对着坏版本打绿灯。CI 每次 push 都跑。
...
Loads the real userscript into a stubbed browser and asserts behaviour. Of the 49 assertions, 34 fail on 2.1 and 3 fail on 3.1, which is what makes the gate meaningful.
