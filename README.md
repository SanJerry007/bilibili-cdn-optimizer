...
它把真实的脚本文件加载进一个桩浏览器（假的 fetch / XHR / localStorage / 时钟），断言的是行为不是结构。75 条断言里，48 条在 2.1 上是红的、18 条在 3.2 上是红的，所以这个闸门不会对着坏版本打绿灯。CI 每次 push 都跑。
...
**判定「卡了」这件事，误判的代价比漏判高**，因为误判会在播放器最饿的时候去抢它的带宽。所以设了四道闸：看门狗必须先亲眼见过一次健康缓冲才武装（否则视频刚开始填缓冲的那几秒缓冲天然就少，会被当成卡，等于把 3.2 刚修掉的问题从后门放回来）；拖进度条前后 4 秒内的卡顿一律不算（拖动必然触发 `waiting`）；缓冲领先量要取**包含播放位置的那一段**，不能一律取最后一段，否则往回拖之后算出来是个大数；视频快播完时缓冲领先本来就接近 0，那不是挨饿。最后两条都是真浏览器实测撞出来的，不是想出来的。
...
Loads the real userscript into a stubbed browser and asserts behaviour. Of the 75 assertions, 48 fail on 2.1 and 18 fail on 3.2, which is what makes the gate meaningful.
